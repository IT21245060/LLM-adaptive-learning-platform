...
const QuizLeaderboard = ({ subject }: { subject: string }) => {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      ...
      try {
        ...
        const res = await getTopUsersByDifficulty(userLevel ?? DifficultyLevels.EASY, 10, subject);
            ...
            res.data.map(async (entry) => {
              ...
              if (userDoc.exists()) {
                profile = {
                  ...
                  photoURL: userDoc.data().photoURL || "/images/default-avatar.png",
                };
              }
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        <CardTitle>Quiz Leaderboard (Level: {userLevel ?? DifficultyLevels.EASY})</CardTitle>
      </CardHeader>
      <CardContent>
        {loading ? (
          ...
          <div className="space-y-4">
            <div className="container mx-auto px-4 pt-8 flex-1">
              <div className="mb-12">
                <TopThreeLeaderboard leaderboard={leaderboard} loading={loading} />
              </div>
            </div>
            <div className="flex flex-col gap-2 container p-4 flex-1 bg-teal-50 rounded-lg mx-auto">
              {leaderboard.length < 4 ? (
                ...
                leaderboard.slice(3).map((entry, index) => (
                  <div
                    key={entry.userId}
                    className={cn("flex items-center justify-between bg-white rounded-lg p-2 px-4", entry.userId === user?.uid && "bg-emerald-400")}
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold">{index + 1}</div>
                      ...
                      <div>
                        <p className="text-sm font-medium leading-none">{entry.userId === user?.uid ? "You" : entry.displayName}</p>
                        <p className={cn("text-xs text-muted-foreground", entry.userId === user?.uid && "text-white")}>{entry.quizCount} quizzes</p>
                      </div>
                    </div>
