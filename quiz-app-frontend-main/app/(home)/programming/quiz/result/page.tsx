...
import { Table, TableBody, TableCell, TableRow } from "@/components/programming/ui/table";
import { getQuizResult } from "@/actions/programming/results";
import { QuizResult } from "@/actions/programming/results";
import { useAuth } from "@/context/AuthContext";
import { useRouter, useSearchParams } from "next/navigation";
import PageLoader from "@/components/programming/shared/loader";
import toast from "react-hot-toast";
import { format } from "date-fns";
import { getQuizGrade, getQuizMarks } from "@/lib/programming/quiz-utils";
import { Button } from "@/components/programming/ui/button";
import ShareResultsModal from "@/components/programming/modals/ShareResultsModal";
import { ExamAndQuestions } from "@/types/programming";
            ...
            {examQuestions.exam.user_id === user?.uid && (
              <div className="flex justify-center items-center mt-8 gap-4 w-sm mx-auto">
                <Button
                  variant="secondary"
                  className="flex-1"
                  onClick={() => router.push(`/programming/quiz/review?id=${id}`)}
                >
                  Review Answers
                </Button>
                <ShareResultsModal />
                <Button
                  className="flex-1"
                  onClick={() => router.push("/programming/dashboard")}
                >
                  Dashboard
                </Button>
              </div>
