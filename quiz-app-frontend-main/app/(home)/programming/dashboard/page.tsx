...
import { Button } from "@/components/programming/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/programming/ui/card";
import { useAuth } from "@/context/AuthContext";
import useUserStore from "@/store/programming/user";
import Image from "next/image";
import React, { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import PageLoader from "@/components/programming/shared/loader";
import RecentActivities from "@/components/programming/dashboard/RecentActivities";
import QuizLeaderboard from "@/components/programming/dashboard/QuizLeaderboard";
import toast from "react-hot-toast";
import QuizStats from "@/components/programming/dashboard/QuizStats";
import { UserDifficultyType } from "@/types/programming";
import apiRequest from "@/lib/programming/api-client";
...
const DashboardPage = () => {
  ...
  //       router.push("/programming/welcome-programming-quiz");
  //     }, 1000);
        ...
        <Card className="col-span-2 p-6">
          ...
          <CardContent className="flex gap-20">
            <div className="flex flex-col flex-1 gap-8">
              ...
              <Button className="w-fit" onClick={() => router.push("/programming/welcome-programming-quiz")}>
  ...
  <h3 className="text-xl font-medium text-gray-700 mb-4 pb-1 border-b border-teal-300">
    Programming Modules
  </h3>
    ...
    {/* Module 1 */}
    {
      moduleStats.length > 0 && (
        moduleStats.map((moduleStat) => {
          return (
            <li key={moduleStat.module} className="bg-white p-4 rounded-lg border-l-4 border-emerald-400 shadow-sm hover:bg-emerald-50 hover:border-emerald-500 transition-all cursor-pointer group">
              <div className="flex justify-between items-start">
                <div>
                  <span className="font-medium text-gray-800 group-hover:text-emerald-600">{moduleStat.module.slice(0, -9)}</span>
              ...
              <div className="mt-2 flex items-center">
                <div className="w-full bg-gray-200 rounded-full h-1.5">
                  <div className="bg-teal-500 h-1.5 rounded-full" style={{width: `${moduleStat.average}%`}}></div>
                </div>
              </div>
            </li>
          )
