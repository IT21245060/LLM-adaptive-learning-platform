...
const QuizPage = () => {
  ...
  const modules = [
    { value: "Database_Architecture", label: "Database Architecture" },
    { value: "Database_Instance", label: "Database Instance" },
    { value: "Multitenant_Architecture", label: "Multitenant Architecture" },
    { value: "Network_Configuration", label: "Network Configuration" },
    { value: "User_Administration", label: "User Administration" },
    { value: "Undo_Data_Management", label: "Undo Data Management" },
    { value: "Database_Security", label: "Database Security" },
    { value: "Data_Consistency_and_Concurrency", label: "Data Consistency and Concurrency" },
    { value: "Backup_and_Recovery", label: "Backup and Recovery" },
    { value: "High_Availability_and_Disaster_Recovery", label: "High Availability and Disaster Recovery" },
  ];
  ...
  const handleStartQuiz = async () => {
    try {
      setIsLoading(true);
      const res = await apiRequest<{ message: string }>("/api/dba/start-quiz/", "POST", {
        user_id: user?.uid,
      });

      // console.log(res);
      if (res && res.data?.message && res.data.message.includes("Quiz started")) {
        const moduleRes = await apiRequest<{ message: string }>("/api/dba/select-module/", "POST", {
          user_id: user?.uid,
        ...
        if (moduleRes && moduleRes.data?.message && moduleRes.data.message.includes("selected for quiz")) {
          /* Set weak topics */
          const lastQuiz = await getLastQuizResultForModule(user?.uid as string, 1, "dba", selectedModule);
    ...
    try {
      ...
      const res = await apiRequest<any>(
        questionType == QuestionType.MCQ
          ? "/api/dba/get-next-question/"
          : questionType == QuestionType.STRUCTURED
          ? "/api/dba/generate-hard-question/"
          : "/api/dba/generate-fill-in-the-blank/",
        "POST",
        {
          user_id: user?.uid,
  ...
  const handleQuizComplete = async (answers: Record<string, string>) => {
    ...
    try {
      ...
      if (user?.uid) {
        ...
        clearStore();
        router.push(`/subjects/dba/quiz/result?id=${savedResult.id}`);
      }
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
